...
(function ($) {
    ...
    $.fn.pwi = function (opts) {
        ...
        if( settings.static == undefined || typeof settings.static == "undefined" )
        {
        ...
        function albums(j) {
            ...
            settings.offset = parseInt(settings.offset);


            if($albumsToShow.length < (settings.offset+1))
            {
                $('.showMoreAlbums-btn').hide();
            }
            else{
                $('.showMoreAlbums-btn').show();
            }
            ...
            $.each($albumsToShow, function(i, n) {
                ...
                //}
                var $scAlbum = parseAlbum({
                    title: ( (n.title.$t.length > settings.showAlbumTitlesLength) ?
                                 n.title.$t.substring(0, settings.showCaptionLength) :
                                 n.title.$t ),
                    photosCount:  ( n.gphoto$numphotos.$t + " " +
                                 ( (n.gphoto$numphotos.$t == "1") ? settings.labels.photo :  settings.labels.photos) ),
                    thumb: n.media$group.media$thumbnail[0].url,
                    id: n.gphoto$id.$t,
                    hidden: ( i > (settings.offset-1) ),
                    static: (settings.static != undefined ? settings.static : false)
                });
                ...
                $('.gallery-container').append($scAlbum);
            });
...
;
    ...
    $.fn.pwi.defaults = {
        ...
        albumThumbSize: 288, //-- specify thumbnail size of albumthumbs (default: 72, supported cropped/uncropped: 32, 48, 64, 72, 104, 144, 150, 160 and uncropped only: 94, 110, 128, 200, 220, 288, 320, 400, 512, 576, 640, 720, 800, 912, 1024, 1152, 1280, 1440, 1600)
        ...
        thumbSize: 200,  //-- specify thumbnail size of photos (default: 72, cropped not supported, supported cropped/uncropped: 32, 48, 64, 72, 104, 144, 150, 160 and uncropped only: 94, 110, 128, 200, 220, 288, 320, 400, 512, 576, 640, 720, 800, 912, 1024, 1152, 1280, 1440, 1600)
